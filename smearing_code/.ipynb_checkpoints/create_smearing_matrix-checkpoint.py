...
def GatherUserInputs(params):
   ...
   if params["interaction_type"] == 2:
      ...
      # Skip flavor selection for CEvNS
   ...
   # MY_COMMENT : Need to add a resolution function for DS 50, maybe add some sort identifier for each detector which then calls the respective res function
...
def GetUnsmearedCEvNS(params, matrixset):
   # User must provide the function here!
   edet_min = params["edet_min"]
   edet_max = params["edet_max"]

   edet_step = 0.00001 # make bins as small as possible
...
if params["interaction_type"] == 4:
    ...
    # Since CEvNS is insensitive to flavor we can add all the flavor matgrices together.

   flavorset = [1,2,3,4,5,6]
   flavornames = ["invalid", "nue", "numu", "nutau", "nuebar", "numubar", "nutaubar"]
   # Run through all 3 flavos



   # Loop over flavors, one matrix for all 6 flavor or have their own matrix
   for iflavor in flavorset:
      our_mat1 = GetUnsmearedCEvNS(params, matrixset, iflavor) # generate the matrix
      our_mat2 = GaussianSmear(params, matrixset, our_mat1) # apply gaussian smearing
      our_mat3 = Downsample(params, our_mat2) # downsample back to requested dims
      our_mat4 = NormalizeEdet(our_mat3) # normalize rows of detected energy
      # Assemble the names using inputs and pre-defined formulae
      path = params["outpath"] + "/"
      channame = "coh_nc_" + flavornames[iflavor] + "_" + params["targetname"] + "_smear"
      filename = "smear_coh_nc_"  + params["targetname"] + "_" + params["detname"] + ".dat"
      WriteMatrix(path + filename, channame, our_mat4)
