...
infile.open("pinched_info.dat");
	...
	while ( getline(infile, inputline) ){
	  ...
	  comment_test = inputline[0];
	   
	  cout << inputline <<endl;
	  cout << "comment test" <<comment_test <<" "<<comment_test.compare("#")<<" "<<inputline[1]<<" "<<inputline[2]<<endl;
	  if (comment_test.compare("#") != 0) {
	    // Parse the string
	    ...
	    i = stoi(subs.c_str());
	    iss >> subs;
	    alpha[0] =  stod(subs.c_str());  
	    iss >> subs;
	    alpha[1] =  stod(subs.c_str());  
	    iss >> subs;
	    alpha[2] =  stod(subs.c_str());  
	    iss >> subs;	    
	    E0[0] =  stod(subs.c_str());  
	    iss >> subs;	    
	    E0[1] =  stod(subs.c_str());  
	    iss >> subs;	    
	    E0[2] =  stod(subs.c_str());  
	    iss >> subs;	    
	    L[0] =  stod(subs.c_str());  
	    iss >> subs;	    
	    L[1] =  stod(subs.c_str());  
